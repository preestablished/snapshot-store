...
| D5 | Commit-race protection | Gate read lock widened to cover parent-check→publish; gated **validating** `register_live_ref` (full dependency-closure check under the gate — registration alone is not protection); sweep finalizes packs AND manifest-unlink batches under gate write with drain-late-roots straggler loops | Closes Race A (put_snapshot presence-check vs sweep drop), Race B (CreateNode/Pin of a pre-fence orphan vs manifest sweep), and the closure holes both reviewers constructed (orphan parent of a delta; CreateNode after the pages' pack finalized; idempotent re-put of a doomed manifest). Full protocol in 02 §1; review trail in 07-review-log.md. |

...
| High | The M8 run was not tied to the qualified NVMe data root; the existing M7 harness uses ordinary tempdirs. | Accepted. `03-wiring-and-performance.md` now requires `M8_STORE_ROOT` or equivalent under the qualified mount and evidence failure if it is not qualified. |

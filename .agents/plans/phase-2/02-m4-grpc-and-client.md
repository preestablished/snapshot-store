...
**Primary path:** the cross-repo request
`~/.agents/projects/control-plane/requests/extend-snapstore-proto-v1/`
(see 00-overview "Cross-repo requests") delivers the full
`determinism.snapstore.v1` service from API.md §1 with prost/tonic codegen in
`determinism-proto`. The request ships the complete `.proto` text (we author
it — control-plane drops it in): all ~20 RPCs, all messages, `NodeMeta` in its
15-field shape, `NodeStatus` enum, and the error-detail messages
`MissingPages { repeated bytes page_hashes; bytes parent_ref }`,
`MissingNodes { repeated uint64 node_ids }`,
`CurrentGeneration { uint64 generation }`.

**Fallback (trigger: request unfulfilled when 01 completes):** vendor
`proto/snapshot_store.proto` at this repo root with local `tonic-build` in
`snapstore-server`/`snapstore-client` `build.rs`, per ARCHITECTURE.md §1; swap
to the published crate when it lands (type paths kept identical via module
re-export so the swap is a Cargo.toml change).
...
| tree | `CreateNode`, `UpdateNodes`, `GetNode`, `GetChildren`, `GetPath`, `QueryNodes` (server-stream, ≤512/msg) | meta WI4 |
...
milestone. **Hypervisor M4 is unblocked when WI1–WI4 + WI6 are green** — WI5
is not on that critical path.
